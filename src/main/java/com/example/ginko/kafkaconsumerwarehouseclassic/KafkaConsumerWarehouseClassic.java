package com.example.ginko.kafkaconsumerwarehouseclassic;
...
public class KafkaConsumerWarehouseClassic {
    public static void main(String[] args) {
        // Create Kafka consumer configuration
        Properties consumerProps = new Properties();
        consumerProps.put("bootstrap.servers", "localhost:9092");  // Kafka broker address
        consumerProps.put("group.id", "warehouse-group");  // Consumer group ID
        ...
        try {
            while (true) {
                ...
                for (ConsumerRecord<String, String> record : records) {
                    System.out.printf("Warehouse consumed message: key = %s, value = %s, partition = %d, offset = %d%n",
                            record.key(), record.value(), record.partition(), record.offset());

                    // Send confirmation message to 'orders-confirmation' topic
                    String confirmationMessage = "Warehouse confirmed order: " + record.value();
                    producer.send(new ProducerRecord<>("orders-confirmation", confirmationMessage));
                    System.out.println("Warehouse sent confirmation: " + confirmationMessage);
                }
            }
        } finally {
